...
def search_contacts(request):
    ...
    search_results = user_contacts.filter(
        ...
        models.Q(birthday__icontains=search_query)
    )
...
@login_required
def birthdays_next_7_days(request):
    if request.method == 'POST':
        ...
        result = []
    return render(request, 'contactsapp/birthdays_next_7_days.html', context={'contacts': result})
...
def detail_contact(request):
    ...
    return render(request, 'contactsapp/detail_contact.html', context={'contacts': page_contacts})
